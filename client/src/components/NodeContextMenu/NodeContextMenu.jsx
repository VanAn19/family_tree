...
const NodeContextMenu = ({ x, y, node, onAddParent, onAddPartner, onAddChild, onViewDetails, onEdit, onDelete, onClose }) => {
  ...
  const hasParents = node.fatherId || node.motherId;
    ...
    <Box position="absolute" top={`${y}px`} left={`${x}px`} ref={menuRef} onBlur={handleBlur} tabIndex={-1}>
      <Menu isOpen={true}>
        <MenuList>
          ...
          {!hasParents && <MenuItem onClick={onAddParent}>Thêm bố/mẹ</MenuItem>}
